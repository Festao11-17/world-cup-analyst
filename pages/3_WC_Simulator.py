...
# Converte immagine in base64 per usarla inline nell'HTML
def flag_b64(s):
    p = flag_path(s)
    if os.path.exists(p):
        with open(p, "rb") as f:
...
def monte_carlo_sim(n, base_seed):
    ...
    def gs(t, col, d=70):
        try:
            return float(rat.get(t, {}).get(col, d))
        except:
            return d
    ...
    def sg(teams):
        ...
        for i in range(len(teams)):
            for j in range(i + 1, len(teams)):
                t1, t2 = teams[i], teams[j]
                p1 = wp(t1, t2) * 100 + random.gauss(0, 8)
                draw_c = max(0, 0.28 - abs(wp(t1, t2) * 100 - 50) * 0.004)
                if random.random() < draw_c:
                    pts[t1] += 1
                    pts[t2] += 1
                elif p1 > 50:
                    pts[t1] += 3
...
# ── SHOW MATCH CARD — fix mobile ─────────────────────────────────────────────
def show_match_card(col, t1, t2, g1, g2, winner, penalties=False, draw=False):
    with col:
        b1 = flag_b64(t1)
        b2 = flag_b64(t2)
        img1 = f"<img src='data:image/png;base64,{b1}' style='width:28px;height:20px;object-fit:cover;border-radius:2px;'>" if b1 else ""
        img2 = f"<img src='data:image/png;base64,{b2}' style='width:28px;height:20px;object-fit:cover;border-radius:2px;'>" if b2 else ""

        wc1 = "#6b7a99" if draw else ("#00d4ff" if winner == t1 else "#6b7a99")
        wc2 = "#6b7a99" if draw else ("#ff3b5c" if winner == t2 else "#6b7a99")
        w1  = "700" if (not draw and winner == t1) else "400"
        w2  = "700" if (not draw and winner == t2) else "400"

        pen_html    = "<div style='font-size:10px;color:#f39c12;margin-top:2px'> Rigori</div>" if penalties else ""
        footer_html = (
            "<div style='margin-top:6px;font-size:10px;color:#6b7a99'> Pareggio</div>"
            if draw else
        ...
        st.markdown(
            f"""
            <div class='wca-card' style='padding:10px;text-align:center;margin-top:4px'>
              <!-- Riga bandiere + score: tutto inline, non usa st.columns -->
              <div style='display:flex;align-items:center;justify-content:space-between;gap:4px;flex-wrap:nowrap;'>
                <!-- squadra 1 -->
                <div style='display:flex;flex-direction:column;align-items:center;gap:4px;flex:1;min-width:0;'>
                  {img1}
                  <span style='font-weight:{w1};font-size:11px;color:{wc1};
                        white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
                        max-width:100%;display:block;'>{t1}</span>
                </div>
                <!-- score centrale -->
                <div style='flex-shrink:0;padding:0 4px;'>
                  <span style='font-family:Bebas Neue,sans-serif;font-size:1.4rem;
                        letter-spacing:2px;white-space:nowrap;'>{g1}-{g2}</span>
                  {pen_html}
                </div>
                <!-- squadra 2 -->
                <div style='display:flex;flex-direction:column;align-items:center;gap:4px;flex:1;min-width:0;'>
                  {img2}
                  <span style='font-weight:{w2};font-size:11px;color:{wc2};
                        white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
                        max-width:100%;display:block;'>{t2}</span>
                </div>
              </div>
              {footer_html}
            </div>
...
run_sim = st.button(" Simula Mondiale 2026", use_container_width=True)
...
if run_sim:
    ...
    for rname in round_names:
        ...
        for t1, t2 in current_round:
            winner, g1, g2, penalties = simulate_match_ko(t1, t2)
    ...
    if champion:
        ...
        with ci:
            ...
            st.markdown(
                ...
                f"<span class='wca-badge' style='margin-left:8px'> {cr.get('AttackRating', '—')} ATT</span>"
                f"<span class='wca-badge' style='margin-left:8px'> {cr.get('DefenseRating', '—')} DEF</span>"
                f"</div>", unsafe_allow_html=True
            )
    ...
    st.markdown("<div style='text-align:center;padding:32px;color:#6b7a99'><div style='font-size:3rem'></div><div style='font-size:16px;margin-top:8px'>Premi il bottone per simulare il Mondiale 2026</div></div>", unsafe_allow_html=True)