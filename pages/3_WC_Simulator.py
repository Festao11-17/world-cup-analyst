...
def monte_carlo_sim(n, base_seed):
    ...
    def sg(teams):
        ...
        for i in range(len(teams)):
            for j in range(i+1,len(teams)):
                ...
                if random.random()<draw_c: pts[t1]+=1;pts[t2]+=1
                elif p1>50: pts[t1]+=3
                else: pts[t2]+=3
                gf[t1]+=1;ga[t2]+=1
    ...
    for i in range(n):
        ...
        for g_teams in GIRONI.values():
            s = sg(g_teams)
            q.append(s[0]); q.append(s[1]); terze_mc.append(s[2])
...
def show_match_card(col, t1, t2, g1, g2, winner, penalties=False, draw=False):
    with col:
        ...
        footer = "<div style='margin-top:6px;font-size:10px;color:#6b7a99'>🤝 Pareggio</div>" if draw else f"<div style='margin-top:6px'><span class='wca-badge' style='font-size:10px'>🏆 {winner}</span></div>"
        st.markdown(
...
if run_sim:
    ...
    for gname, teams in GIRONI.items():
        ...
        qualificate.append(standing[0]); qualificate.append(standing[1])
