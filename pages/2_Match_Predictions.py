...
with col1:
    search1 = st.text_input("🔍 Cerca Squadra Casa", placeholder="Es. Brasile...")
    filtered1 = [t for t in teams if search1.lower() in t.lower()] if search1 else teams
    team1 = st.selectbox("Squadra Casa", filtered1, key="t1")
with col2:
    search2 = st.text_input("🔍 Cerca Squadra Ospite", placeholder="Es. Francia...")
    filtered2 = [t for t in teams if search2.lower() in t.lower()] if search2 else teams
    team2 = st.selectbox("Squadra Ospite", filtered2, key="t2")
...
st.markdown(
    ...
    f"</div>"
    f"<div style='display:flex;justify-content:space-between;font-size:11px;color:#6b7a99;margin-bottom:8px'>"
...
for label, r1, r2 in [
    ...
    c1_col = "#00d4ff" if float(r1) >= float(r2) else "#6b7a99"
    c2_col = "#ff3b5c" if float(r2) > float(r1) else "#6b7a99"
    st.markdown(
        f"<div style='margin-bottom:14px'>"
        f"<div style='display:flex;justify-content:space-between;margin-bottom:4px'>"
        f"<span style='font-weight:700;color:{c1_col}'>{r1}</span>"
        f"<span style='font-size:12px;color:#6b7a99'>{label}</span>"
        f"<span style='font-weight:700;color:{c2_col}'>{r2}</span>"
        f"</div>"
